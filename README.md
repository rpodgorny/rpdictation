...
- **[OpenWhispr](https://github.com/OpenWhispr/openwhispr)** — A cross-platform (Windows, macOS, Linux) open-source dictation app with true push-to-talk via a native low-level keyboard hook, system-wide in any app. Supports the same cloud providers as rpdictation (OpenAI, Groq, Mistral) with BYOK, plus local transcription via whisper.cpp and NVIDIA Parakeet (sherpa-onnx, 25 languages including Czech), and offers AI cleanup post-processing. It ships as an Electron desktop app with its own global hotkey manager and GUI configuration. rpdictation is Linux-only today and differs in being a single Rust binary driven from the shell: it integrates with your window manager's existing keybind system rather than owning the hotkey, exposes `start`/`stop`/`toggle` subcommands for scripting, chains providers with automatic fallback (including Google's free Chromium key as a last resort), and is a fraction of the runtime footprint.
